...
def prediction():
    ...
    try:
        ...
        return error({"code": "ner.json", "text": "The input could not be parsed into json",
                      "detail": {'traceback': traceback.format_exc()}})

    try:
        df = pd.DataFrame(data={"text": [text["content"]]})
    except:
        return error({"code": "ner.missing.content", "text": "There is no content in the input",
                      "detail": {'traceback': traceback.format_exc()}})
    ...
    try:
        ...
        return error(
            {"code": "ner.model", "text": "Error response from model", "detail": {'traceback': traceback.format_exc()}})
